    ...
    if (body.reorder && Array.isArray(body.orderedIds)) {
      ...
      await prisma.$transaction(
        parsed.data.orderedIds.map((moduleId, index) =>
          prisma.courseModule.update({ where: { id: moduleId }, data: { order: index + 1 } })
        )
      );
...
export async function DELETE(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const remaining = await prisma.courseModule.findMany({ where: { courseId: module.courseId }, orderBy: { order: "asc" } });
    await prisma.$transaction(
      remaining.map((m, index) => prisma.courseModule.update({ where: { id: m.id }, data: { order: index + 1 } }))
    );
