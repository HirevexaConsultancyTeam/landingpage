...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    const parsed = categorySchema.safeParse({
      ...body,
      slug: slugify(body.slug || body.name, { lower: true, strict: true, trim: true }),
    });
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      return NextResponse.json({ message: "Duplicate category." }, { status: 409 });
    }
...
export async function DELETE(req: NextRequest, { params }: Params) {
  try {
    ...
    if (category._count.courses > 0) {
      return NextResponse.json(
        { message: "This category contains courses. Move or delete the courses before deleting the category." },
        { status: 400 }
      );
    }
